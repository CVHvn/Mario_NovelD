...
from src.simple_pixel_hash import *
...
class Agent():
    def __init__(self, envs, world, stage, action_type, num_envs, state_dim, action_dim, save_dir, save_model_step,
                 ...
                 ext_adv_coef, additional_bonus_state_8_4_option, update_proportion, norm_rnd_output, device):
        self.world = world
        self.stage = stage
        ...
        self.norm_rnd_output = norm_rnd_output
        ...
        self.rnd_current_states = np.zeros((1000,)).reshape(-1)
        self.rnd_next_states = np.zeros((1000,)).reshape(-1)
        self.rnd_current_states_norm = np.zeros((1000,)).reshape(-1)
        self.rnd_next_states_norm = np.zeros((1000,)).reshape(-1)
        self.rewards_int_idx = 0
        self.rewards_int_len = 0

        self.rnd_loss = np.zeros((1000,)).reshape(-1)
        self.rnd_loss_idx = 0
        self.rnd_loss_len = 0

        self.obs_rms = RunningMeanStd(shape=(1, 1, 84, 84), device=self.device)
        self.dm_rms = RunningMeanStd(shape=(1,))
        self.rnd_rms = RunningMeanStd(shape=(1,))
...
loss_e: {(self.E_loss.sum()/self.len_loss):.4f} loss_rnd: {(self.rnd_loss.sum()/self.rnd_loss_len):.4f} \
reward_int: {(self.rewards_int.sum()/self.rewards_int_len):.4f} \
rnd_current_state: {(self.rnd_current_states.sum()/self.rewards_int_len):.4f} \
rnd_next_state: {(self.rnd_next_states.sum()/self.rewards_int_len):.4f} \
rnd_current_state_norm: {(self.rnd_current_states_norm.sum()/self.rewards_int_len):.4f} \
rnd_next_state_norm: {(self.rnd_next_states_norm.sum()/self.rewards_int_len):.4f} \
episode_time: {datetime.now() - episode_time}\n')
    ...
    def update_reward_int_log(self, rnd_current_states, rnd_next_states, rnd_current_states_norm, rnd_next_states_norm, rewards_int):
        for i in range(len(rewards_int)):
            self.rewards_int[self.rewards_int_idx] = rewards_int[i].mean()
            self.rnd_current_states[self.rewards_int_idx] = rnd_current_states[i].mean()
            self.rnd_next_states[self.rewards_int_idx] = rnd_next_states[i].mean()
            self.rnd_current_states_norm[self.rewards_int_idx] = rnd_current_states_norm[i].mean()
            self.rnd_next_states_norm[self.rewards_int_idx] = rnd_next_states_norm[i].mean()
            self.rewards_int_idx = (self.rewards_int_idx + 1)%1000
            self.rewards_int_len = min(self.rewards_int_len+1, 1000)

    def update_rnd_log(self, loss_rnd):
        self.rnd_loss[self.rnd_loss_idx] = loss_rnd
        self.rnd_loss_idx = (self.rnd_loss_idx + 1)%1000
        self.rnd_loss_len = min(self.rnd_loss_len+1, 1000)
    ...
    def compute_reward_int_rnd(self, next_state):
        ...
        return rewards_int.reshape(-1).cpu().numpy()

    def learn(self):
        # get all data
        states, actions, next_states, rewards, rnd_current_states, rnd_next_states, rnd_current_states_norm, rnd_next_states_norm, rewards_int, dones, old_logits, old_values, old_values_int = self.memory.get_data()
        ...
        rnd_current_states = np.array(rnd_current_states)
        rnd_next_states = np.array(rnd_next_states)
        rnd_current_states_norm = np.array(rnd_current_states_norm)
        rnd_next_states_norm = np.array(rnd_next_states_norm)
        rewards_int = np.array(rewards_int)

        self.update_reward_int_log(rnd_current_states, rnd_next_states, rnd_current_states_norm, rnd_next_states_norm, rewards_int)
        ...
        for epoch in range(self.num_epoch):
            ...
            for i in range(len(old_values)//self.batch_size):
                ...
                if early_stopping == False:
                    ...
                    break
            # ensure embedding batchsize large enough (when use mask with self.update_proportion as RND, because update_proportion very small, we any have 0-3 samples per update!)
            ...
            for i in range(num_sample_embeddings//self.batch_size + int(num_sample_embeddings % self.batch_size != 0)):
                ...
                loss_rnd = self.train_rnd(batch_next_states)

                self.update_rnd_log(loss_rnd)

            if early_stopping:
                break

    def train_rnd(self, next_states):
        self.feature_optimizer.zero_grad()
        with torch.no_grad():
            target_features = self.target_model(next_states)
        features = self.predict_model(next_states)
        loss_rnd = ((features - target_features)**2).mean()
        loss_rnd.backward()
    ...
    def train(self):
        ...
        episodic_memorys = [SimplePixelHash() for i in range(self.num_envs)]
        ...
        while True:
            ...
            check_next_states = np.zeros(self.num_envs)

            for i in range(self.num_envs):
                episodic_memorys[i].update(states[i][3])
                check_next_states[i] = int(episodic_memorys[i].count(next_states[i][3]) == 0)

            rnd_current_states = self.compute_reward_int_rnd(states)
            rnd_next_states = self.compute_reward_int_rnd(next_states)
            rnd_current_states_norm = rnd_current_states / (np.sqrt(self.rnd_rms.var.cpu().numpy()) + 1e-11)
            rnd_next_states_norm = rnd_next_states / (np.sqrt(self.rnd_rms.var.cpu().numpy()) + 1e-11)
            if self.current_step > self.learn_step * 10:
                self.rnd_rms.update(torch.tensor(rnd_next_states).reshape(-1))

            if self.norm_rnd_output:
                rewards_int = np.maximum(rnd_next_states_norm - 0.5 * rnd_current_states_norm, 0) * check_next_states
            else:
                rewards_int = np.maximum(rnd_next_states - 0.5 * rnd_current_states, 0) * check_next_states

            # save to memory
            self.memory.save(states, actions, rewards, rnd_current_states, rnd_next_states,
                             rnd_current_states_norm, rnd_next_states_norm, rewards_int, next_states, dones, logit, value, value_int)
...
loss_v_int: {(self.V_int_loss.sum()/self.len_loss):.4f} loss_e: {(self.E_loss.sum()/self.len_loss):.4f} \
loss: {(self.total_loss.sum()/self.len_loss):.4f} approx_kl_div: {(self.approx_kl_divs.sum()/self.len_loss):.4f} \
loss_rnd: {(self.rnd_loss.sum()/self.rnd_loss_len):.4f} \
reward_int: {(self.rewards_int.sum()/self.rewards_int_len):.4f} \
rnd_current_state: {(self.rnd_current_states.sum()/self.rewards_int_len):.4f} \
rnd_next_state: {(self.rnd_next_states.sum()/self.rewards_int_len):.4f} \
rnd_current_state_norm: {(self.rnd_current_states_norm.sum()/self.rewards_int_len):.4f} \
