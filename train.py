...
def get_args():
    ...
    parser.add_argument('--update_proportion', type=float, default=0.25, help='Update proportion coefficient')
    parser.add_argument('--norm_rnd_output', type=bool, default=True, help='Use normalize rnd output or not')
...
def train(config):
    ...
    agent = Agent(envs = envs, world = config.world, stage = config.stage, action_type = config.action_type, num_envs = config.num_envs,
              ...
              model = model, target_model = target_model, predict_model = predict_model,
              gamma = config.gamma, gamma_int = config.gamma_int,
              ...
              int_adv_coef = config.int_adv_coef, update_proportion = config.update_proportion, norm_rnd_output = config.norm_rnd_output,
